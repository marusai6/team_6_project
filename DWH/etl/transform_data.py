...
def clean_data(df, table_name):
    ...
    if table_name == 'базы_данных':
        ...
        valid_user_ids = pd.read_sql("SELECT id FROM avakyan_dds.сотрудники_дар", engine)['id']
        ...
        valid_fr = pd.read_sql("SELECT id FROM avakyan_dds.базы_данных", engine)['id']
        ...
        print(df.shape)
        valid_kn = pd.read_sql("SELECT id FROM avakyan_dds.уровни_знаний_в_отрасли", engine)['id']
        ...
        valid_user_ids = pd.read_sql("SELECT id FROM avakyan_dds.сотрудники_дар", engine)['id']
        df = df[df['User ID'].isin(valid_user_ids)]
        valid_kn = pd.read_sql("SELECT id FROM avakyan_dds.уровни_знаний_в_предметной_област", engine)['id']
        ...
        valid_kn = pd.read_sql("SELECT id FROM avakyan_dds.уровни_знаний", engine)['id']
        ...
        valid_fr = pd.read_sql("SELECT id FROM avakyan_dds.фреймворки", engine)['id']
        ...
        valid_fr = pd.read_sql("SELECT id FROM avakyan_dds.языки_программирования", engine)['id']
        ...
        valid_fr = pd.read_sql("SELECT id FROM avakyan_dds.инструменты", engine)['id']
...
def load_and_clean_table(table_name):
    ...
    df_cleaned.to_sql(table_name, engine, schema='avakyan_dds', if_exists='append', index=False)

    if not df_broken.empty:
        broken_table_name = f"{table_name}_broken"
        df_broken.to_sql(broken_table_name, engine, schema='avakyan_dds', if_exists='replace', index=False)
...
print("All tables processed and loaded into avakyan_dds successfully.")